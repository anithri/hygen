...
exports.yargsResolver = function (config) {
    return Promise.resolve(yargs_1.default(config.env.argv)
        .parserConfiguration({
        'combine-arrays': true,
    })
        .scriptName('hygen')
        ...
        .showHidden('verbose', 'show all the help').argv).then(function (yargv) {
        return mergeConfig_1.mergeConfig(config, {
            yargv: yargv,
            params: yargv,
        });
    });
