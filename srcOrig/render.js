...
var renderTemplate = function (tmpl, locals, config) {
    return typeof tmpl === 'string' ? ejs.render(tmpl, context(locals, config)) : tmpl;
};
...
var render = function (args, config) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, getFiles(args.actionfolder)
                    // TODO: add a test to verify this sort
                    .then(function (things) { return things.sort(function (a, b) { return a.localeCompare(b); }); })
                    // TODO: add a test for ignoring prompt.js and index.js
                    .then(filter(function (f) { return !ignores.find(function (ig) { return f.endsWith(ig); }); }))
                    .then(filter(function (file) { return (args.subaction ? file.match(args.subaction) : true); }))
                    .then(map(function (file) {
                    return fs.readFile(file).then(function (text) { return ({ file: file, text: text.toString() }); });
                }))
                    .then(function (_) { return Promise.all(_); })
                    .then(map(function (_a) {
                    var file = _a.file, text = _a.text;
                    return Object.assign({ file: file }, fm(text));
                }))
                    .then(map(function (_a) {
                    var file = _a.file, attributes = _a.attributes, body = _a.body;
                    return ({
                        file: file,
                        attributes: Object.entries(attributes).reduce(function (obj, _a) {
                            var key = _a[0], value = _a[1];
                            return (obj[key] = renderTemplate(value, args, config)) && obj;
                        }, {}),
